...
function loginn() {
	var filelogin = fs.readdirSync(path.join(__dirname, "..", "../Plugin/Acc"))
	if(filelogin.length == 0){
		...
		for (var i = 0; i < filelogin.length; i++) {
				if (filelogin[i].endsWith(".json")) {
					...
					var json = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "../Plugin/Acc", file), 'utf8'));
					if (json.url && json.cookies) {
						...
						return login({appState: JSON.parse(JSON.stringify(appstate))}, (err, api) => {
							if(err){
								...
								fs.unlinkSync(path.join(__dirname, "..", "../Plugin/Acc", file))
								return process.exit()
							}
					...
					var atp = fs.readFileSync(path.join(__dirname, "..", "../Plugin/Acc", filelogin[i]), 'utf8');
					...
					if (validCount >= validItems.length) {
						...
						return login({appState: JSON.parse(JSON.stringify(unofficialAppState))}, (err, api) => {
							if(err){
								console.error(JSON.stringify(err))
								console.error('Not logged in');
								fs.unlinkSync(path.join(__dirname, "..", "../Plugin/Acc", filelogin[i]))
								return process.exit()
							}
							console.log('[FACEBOOK] Logged in');
							return load(api)
